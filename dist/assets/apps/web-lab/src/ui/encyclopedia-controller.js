...
export class EncyclopediaController {
    ...
    constructor(database) {
        ...
        byId("searchInput").addEventListener("input", () => this.render());
        byId("encyclopediaType").addEventListener("change", () => this.render());
        this.render();
    }
    render() {
        const type = byId("encyclopediaType").value;
        const query = byId("searchInput").value.trim().toLocaleLowerCase("es");
        let cards = [];
        if (type === "amino") {
        ...
        byId("encyclopediaGrid").innerHTML = cards.join("") || '<article class="info-card"><h3>Sin resultados</h3><p>Prueba otra búsqueda.</p></article>';
    }
}
