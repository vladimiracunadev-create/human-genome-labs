...
function makeQuestion(mode, text, visual, correct, pool, explanation, random) {
...
export function resolveGameMode(mode, random = Math.random) {
    if (mode !== "mixed")
        return mode;
    return sample(["codon_to_amino", "amino_to_codon", "dna_to_rna", "complement"], random);
}
export function createQuestion(mode, database, random = Math.random) {
    ...
    if (resolved === "amino_to_codon") {
        ...
        return makeQuestion(resolved, `¿Cuál de estos codones puede producir ${amino?.name_es ?? code} (${code})?`, code, correct, pool, `${amino?.name_es ?? code} puede ser codificada por: ${valid.join(", ")}.`, random);
    }
    if (resolved === "dna_to_rna") {
        const target = sample(database.codons, random);
        return makeQuestion(resolved, "Transcribe este codón de ADN a ARN mensajero.", target.dna, target.rna, database.codons.map((codon) => codon.rna), `Durante la transcripción, T se representa como U: ${target.dna} → ${target.rna}.`, random);
    }
    const target = sample(database.codons, random).dna;
    const correct = complementDna(target);
    return makeQuestion(resolved, "¿Cuál es la secuencia complementaria de este ADN?", target, correct, database.codons.map((codon) => codon.dna), `A se aparea con T y C con G: ${target} ↔ ${correct}.`, random);
}
export function calculateQuestionScore(correct, previousStreak, base, bonus) {
    if (!correct)
        return 0;
