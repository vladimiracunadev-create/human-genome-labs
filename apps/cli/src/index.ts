...
import { createScientificResult, type ModuleManifest } from "../../../packages/scientific-contracts/src/index.js";
import {
...
interface Registry { schemaVersion: string; projectVersion: string; modules: ModuleManifest[]; capabilities: Array<Record<string, unknown> & { id: string; moduleId: string; maturity: string }>; }

const root = resolve(import.meta.dirname, "../../../..");
const database = JSON.parse(await readFile(resolve(root, "packages/biology-data/data/biology.json"), "utf8")) as BiologyDatabase;
const registry = JSON.parse(await readFile(resolve(root, "registry/modules.json"), "utf8")) as Registry;
const profiles = JSON.parse(await readFile(resolve(root, "packages/organism-profiles/data/profiles.json"), "utf8")) as OrganismProfileDatabase;
...
function printOutput(output: unknown, asJson: boolean): void {
  ...
  else console.dir(output, { depth: null, colors: process.stdout.isTTY });
}

function moduleById(id: string): ModuleManifest {
  const found = registry.modules.find((item) => item.id === id);
  if (!found) throw new Error(`Módulo no registrado: ${id}`);
  return found;
}

function result<T>(moduleId: string, analysis: string, value: T, options: {
  source?: string; evidenceLevel?: "observed" | "experimentally_supported" | "computational_prediction" | "hypothesis" | "unknown";
  parameters?: Record<string, unknown>; warnings?: string[]; limitations?: string[]; status?: "success" | "warning" | "error";
} = {}) {
...
if (command === "profiles") {
  ...
  if (!selected.length) throw new Error(`No existe el perfil '${id}'.`);
...
if (command === "frontiers") {
  ...
  if (!frontiers.length) throw new Error(`No existe la frontera '${domain}'.`);
...
if (!filename || filename.startsWith("--")) usage();
...
if (!records.length) throw new Error("El archivo no contiene secuencias FASTA.");
const output = records.map((record) => {
  if (command === "summary") return result("sequence-analysis", "sequence.summary", { id: record.id, description: record.description, ...summarizeSequence(record.sequence) }, { source: filename });
  if (command === "validate") return result("sequence-analysis", "sequence.validate", { id: record.id, ...validateDna(record.sequence) }, { source: filename, evidenceLevel: "observed" });

  const code = (option(args, "--code", "standard") ?? "standard") as GeneticCodeId;
  const frame = Number(option(args, "--frame", "1")) as 1 | 2 | 3;
  if (![1, 2, 3].includes(frame)) throw new Error("--frame debe ser 1, 2 o 3.");

  if (command === "translate") {
    if (args.includes("--six-frames")) return result("sequence-analysis", "sequence.translation.six_frames", { id: record.id, translations: translateSixFrames(record.sequence, database, code) }, { source: filename, parameters: { code } });
    const strand = option(args, "--strand", "+") as "+" | "-";
    if (!["+", "-"].includes(strand)) throw new Error("--strand debe ser + o -.");
  ...
  if (command === "codon-usage") return result("sequence-analysis", "sequence.codon_usage", { id: record.id, usage: codonUsage(record.sequence, database, frame, code).filter((item) => item.count > 0) }, { source: filename, parameters: { frame, code } });
  if (command === "variant") {
    const position = Number(option(args, "--position"));
    const reference = option(args, "--ref");
    const alternate = option(args, "--alt");
    if (!Number.isInteger(position) || reference === undefined || alternate === undefined) throw new Error("variant requiere --position, --ref y --alt.");
  ...
  if (command === "structures") {
    ...
    if (!Number.isInteger(maxLoop) || maxLoop < 1 || maxLoop > 20) throw new Error("--max-loop debe ser un entero entre 1 y 20.");
